...
export default function CustomerDashboard() {
  ...
  const fetchAccount = async () => {
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/accounts/${user.user_id}`);
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/transactions/history/${user.user_id}`);
      const data = await res.json();
  ...
  const transferMoney = async () => {
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/transactions/transfer`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const sendPdfToEmail = async () => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/transactions/send-report`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
